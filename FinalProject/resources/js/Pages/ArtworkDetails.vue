...
const editLink = computed(() => route('admin.artwork.edit', props.artwork.id))

const deleteEndpoint = computed(() => route('admin.artwork.destroy', props.artwork.id))
...
const deleteArtwork = () => {
  ...
  axios.delete(deleteEndpoint.value)
    .then(() => {
      const redirectTo = '/theArt/artCatalog'
...
<template>
  ...
  <div class="catalogContent">
    <div class="artDetails">
      ...
      <p><strong>Artist:</strong> <a :href="`/theArt/${artwork.artist.id}/artistDetail`" class="artistLink">{{ artwork.artist.name }}</a></p>
      <p><strong>Style:</strong> {{ artwork.style?.name ?? 'Unknown' }}</p>
      <p><strong>Year Created:</strong> {{ artwork.year_created }}</p>
      <p v-if="isAdmin"><strong>Views:</strong> {{ artwork.views.toLocaleString() }}</p>
      <p class="artDescription">{{ artwork.description }}</p>
    </div>
